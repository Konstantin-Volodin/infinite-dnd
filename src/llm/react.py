"""Agent for resolving character actions into world consequences."""
...
agent: Agent[ReactDeps, str] = Agent(
    model=create_model(),
    deps_type=ReactDeps,
    output_type=ToolOutput(str, name="done"),
    instructions="Resolve exactly one character action into concrete consequences.",
)


@agent.system_prompt
def identity(_: RunContext[ReactDeps]) -> str:
    return react_system()


@agent.instructions
def context(ctx: RunContext[ReactDeps]) -> str:
